...
#include <wsda_device_module/module_dll.h>
...
using namespace daq;
using WSDADeviceModuleTest = testing::Test;
using namespace test_config_provider_helpers;
using WSDADeviceModuleTestConfig = ConfigProviderTest;
...
TEST_F(WSDADeviceModuleTest, CreateModule)
{
...
TEST_F(WSDADeviceModuleTest, ModuleName)
{
    auto module = CreateModule();
    ASSERT_EQ(module.getName(), "WSDADeviceModule");
}

TEST_F(WSDADeviceModuleTest, VersionAvailable)
{
    auto module = CreateModule();
    ASSERT_TRUE(module.getVersionInfo().assigned());
}

TEST_F(WSDADeviceModuleTest, VersionCorrect)
{
    auto module = CreateModule();
    auto version = module.getVersionInfo();

    ASSERT_EQ(version.getMajor(), WSDA_DEVICE_MODULE_MAJOR_VERSION);
    ASSERT_EQ(version.getMinor(), WSDA_DEVICE_MODULE_MINOR_VERSION);
    ASSERT_EQ(version.getPatch(), WSDA_DEVICE_MODULE_PATCH_VERSION);
}

TEST_F(WSDADeviceModuleTest, EnumerateDevices)
{
    auto module = CreateModule();
    ...
    ASSERT_EQ(deviceInfo[0].getConnectionString(), "daqwsda://device0");
    ASSERT_EQ(deviceInfo[1].getConnectionString(), "daqwsda://device1");
}

TEST_F(WSDADeviceModuleTest, CreateDeviceConnectionStringNull)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, CreateDeviceConnectionStringEmpty)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, CreateDeviceConnectionStringInvalid)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, CreateDeviceConnectionStringInvalidId)
{
    auto module = CreateModule();

    ASSERT_THROW(module.createDevice("daqwsda://devicett3axxr1", nullptr), InvalidParameterException);
}

TEST_F(WSDADeviceModuleTest, CreateDeviceConnectionStringOutOfRange)
{
    auto module = CreateModule();

    ASSERT_THROW(module.createDevice("daqwsda://device3", nullptr), NotFoundException);
}

TEST_F(WSDADeviceModuleTest, CreateDeviceConnectionStringCorrect)
{
    auto module = CreateModule();

    DevicePtr device;
    ASSERT_NO_THROW(device = module.createDevice("daqwsda://device1", nullptr));
}

TEST_F(WSDADeviceModuleTest, DeviceDomainResolution)
{
    auto module = CreateModule();

    auto device = module.createDevice("daqwsda://device1", nullptr);
...
TEST_F(WSDADeviceModuleTest, DeviceDomainUnit)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, DeviceDomainTicksSinceEpoch)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, DeviceDomainOrigin)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, GetAvailableComponentTypes)
{
    ...
    ASSERT_TRUE(deviceTypes.hasKey("daqwsda"));
    ASSERT_EQ(deviceTypes.get("daqwsda").getId(), "daqwsda");
...
TEST_F(WSDADeviceModuleTest, CreateFunctionBlockIdNull)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, CreateFunctionBlockIdEmpty)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, DeviceNumberOfChannels)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, DeviceChangeNumberOfChannels)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, DeviceChangeAcqLoopTime)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, DeviceGlobalSampleRate)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, ChannelWaveform)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
...
TEST_F(WSDADeviceModuleTest, ChannelFrequency)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
...
TEST_F(WSDADeviceModuleTest, ChannelDC)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
...
TEST_F(WSDADeviceModuleTest, ChannelAmplitude)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
...
TEST_F(WSDADeviceModuleTest, ChannelName)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
...
TEST_F(WSDADeviceModuleTest, ChannelNoiseAmplitude)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
...
TEST_F(WSDADeviceModuleTest, ChannelCustomRange)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
...
TEST_F(WSDADeviceModuleTest, ChannelSampleRate)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
...
TEST_F(WSDADeviceModuleTest, CoerceChannelSampleRate)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
...
TEST_F(WSDADeviceModuleTest, Ids)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
    ...
    ASSERT_EQ(channel.getLocalId(), "WSDACh0");
    ASSERT_EQ(channel.getGlobalId(), "/WSDADev1/IO/AI/WSDACh0");

    ASSERT_EQ(valueSignal.getLocalId(), "AI0");
    ASSERT_EQ(valueSignal.getGlobalId(), "/WSDADev1/IO/AI/WSDACh0/Sig/AI0");

    ASSERT_EQ(domainSignal.getLocalId(), "AI0Time");
    ASSERT_EQ(domainSignal.getGlobalId(), "/WSDADev1/IO/AI/WSDACh0/Sig/AI0Time");
}
...
TEST_F(WSDADeviceModuleTest, ChannelProperties)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
...
TEST_F(WSDADeviceModuleTest, SignalCheck)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
...
TEST_F(WSDADeviceModuleTest, DeviceRemoveDisconnectsInputPort)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
...
TEST_F(WSDADeviceModuleTest, ChannelRemovedDisconnectsInputPort)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
    auto channels = device.getChannels();
...
TEST_F(WSDADeviceModuleTest, CreateDeviceTwice)
{
    auto module = CreateModule();

    DevicePtr device;
    ASSERT_NO_THROW(device = module.createDevice("daqwsda://device1", nullptr));
    ASSERT_THROW(module.createDevice("daqwsda://device1", nullptr), AlreadyExistsException);
}

TEST_F(WSDADeviceModuleTest, CreateReleaseAndCreateDevice)
{
    auto module = CreateModule();
    ...
    ASSERT_NO_THROW(device = module.createDevice("daqwsda://device1", nullptr));
    device.release();
    ASSERT_NO_THROW(module.createDevice("daqwsda://device1", nullptr));
}

TEST_F(WSDADeviceModuleTest, Folders)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
...
TEST_F(WSDADeviceModuleTest, Sync)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
...
TEST_F(WSDADeviceModuleTest, Serialize)
{
    auto module = CreateModule();
    auto device = module.createDevice("daqwsda://device1", nullptr);
...
TEST_F(WSDADeviceModuleTest, DeviceEnableCANChannel)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, ReadCANChannel)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, ReadCANChannelWithStreamReader)
{
    const auto module = CreateModule();

    const auto device = module.createDevice("daqwsda://device1", nullptr);
...
TEST_F(WSDADeviceModuleTest, ReadAIChannelWithFixedPacketSize)
{
    const auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTest, ReadConstantRule)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTestConfig, JsonConfigReadWSDADeviceLocalId)
{
    std::string filename = "test.json";
    std::string json = "{ \"Modules\": { \"WSDADevice\": { \"LocalId\": \"testtest\" } } }";
    ...
    getChildren(getChildren(expectedOptions, "Modules"), "WSDADevice").set("LocalId", "testtest");
...
TEST_F(WSDADeviceModuleTestConfig, DeviceModuleJsonConfigLocalId)
{
    std::string filename = "test.json";
    std::string json = "{ \"Modules\": { \"WSDADevice\": { \"LocalId\": \"testtest\" } } }";
    createConfigFile(filename, json);
    ...
    DevicePtr ptr;
    ASSERT_NO_THROW(ptr = module.createDevice("daqwsda://device1", nullptr));
    ASSERT_EQ(ptr.getLocalId(), "testtest");
}

TEST_F(WSDADeviceModuleTestConfig, DeviceModuleJsonConfigName)
{
    std::string filename = "test.json";
    std::string json = "{ \"Modules\": { \"WSDADevice\": { \"Name\": \"testname\" } } }";
    createConfigFile(filename, json);
...
TEST_F(WSDADeviceModuleTestConfig, DeviceModuleJsonConfigLocalIdAndName)
{
    std::string filename = "test.json";
    std::string json = "{ \"Modules\": { \"WSDADevice\": { \"LocalId\": \"testtest\", \"Name\": \"testname\" } } }";
    createConfigFile(filename, json);
...
TEST_F(WSDADeviceModuleTestConfig, DeviceModuleJsonConfigMalformed)
{
    std::string filename = "test.json";
    std::string json = "{ \"Modules\": { \"WSDADevice\": { \"LocalId\": { \"Error\": \"testtest\" } } } }";
    createConfigFile(filename, json);
    ...
    ASSERT_THROW(module.createDevice("daqwsda://device1", nullptr), NoInterfaceException);
}

TEST_F(WSDADeviceModuleTestConfig, DeviceModuleJsonConfigDefault)
{
    auto options = GetDefaultOptions();
    ...
    DevicePtr ptr;
    ASSERT_NO_THROW(ptr = module.createDevice("daqwsda://device1", nullptr));
    ASSERT_EQ(ptr.getLocalId(), "WSDADev1");
}

TEST_F(WSDADeviceModuleTestConfig, DeviceModuleJsonConfigNoOptions)
{
    auto module = CreateModule();
...
TEST_F(WSDADeviceModuleTestConfig, DeviceModuleJsonConfigEmptyString)
{
    auto options = GetOptionsWithReferenceDevice();
    ...
    DevicePtr ptr;
    ASSERT_THROW(ptr = module.createDevice("daqwsda://device1", nullptr), GeneralErrorException);
}
